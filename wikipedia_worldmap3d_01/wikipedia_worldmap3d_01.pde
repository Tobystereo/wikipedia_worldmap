 ...
 *
 *  TODO: 
 *  implement all languages (fix csv's)
 *  fix 3D mapping currently only half a sphere 
 *  create buffer functino that reads and stores the coordinates when languages are switched so that the file doesn't have to be parsed every time the map is rendered.
...
//  if (key=='s' || key=='S') saveFrame(timestamp()+"_##.png");
  if (key=='p' || key=='P') {doLoop = true; savePDF = true; pointSize = 0.001;changeLanguage(language);}
  if (keyCode == 'd') {
    println("3D");
    background(0,0,100);
    threeD=true; 
